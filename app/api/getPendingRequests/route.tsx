...
const getToken = async (address: string, id: number, rpc: string) => {
  ...
  const tokens = [
    ...evmChainData.map((x) => x.tokens),
    ...solanaChainData.map((x) => x.tokens),
    ...aptosChainData.map((x) => x.tokens),
  ].flat()
...
export async function GET(req: Request) {
  ...
  console.log(
    "🍕 ",
    address,
    `${process.env.FETCCH_BASE_URL}/transaction-request?payer=${address}`
  )

  const myReq = await fetch(
    `${process.env.FETCCH_BASE_URL}/transaction-request?receiver=${address}`,
    {
      method: "GET",
      headers: {
  ...
  for (let i = 0; i < res.data.length; i++) {
    ...
    for (let j = 0; j < req.actions.length; j++) {
      ...
      const chain = [...evmChainData, ...solanaChainData].find(
        (chain) => chain.id === action.chain
      )!
